                ...
                <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
                  ...
                  {dbDeleteTarget===l.id?(
                    <div style={{display:"flex",gap:5,alignItems:"center",flexWrap:"wrap"}}>
                      ...
                      <button onClick={async()=>{if(dbPw===ADMIN_USER.password){try{const r=await fetch("/api/lessons?action=delete",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({lessonId:l.id})});const d=await r.json().catch(()=>({}));if(!r.ok&&r.status!==404){setDbPwError(d.error||"Delete failed ("+r.status+")");return;}onDeleteLesson(l.studentEmail,l.id);setDbDeleteTarget(null);setDbPw("");}catch(e){// Network error — server-side delete likely succeeded, remove from UI anyway
onDeleteLesson(l.studentEmail,l.id);setDbDeleteTarget(null);setDbPw("");}}else{setDbPwError("Wrong password");}}} style={{background:"#dc2626",color:"white",border:"none",padding:"3px 10px",borderRadius:50,cursor:"pointer",fontSize:"0.73rem",fontWeight:700}}>Delete</button>
