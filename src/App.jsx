...
export default function App(){
  ...
  const cancelLesson=async(id)=>{
    ...
    const sendEmail3=(to,subject,text)=>fetch("/api/send-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to,subject,text})}).catch(()=>{});
    await sendEmail3(user.email,"Lesson Cancelled - "+fmtDateShort(lesson.date),"Hi "+user.name+",\n\n"+cancelMsg);
    await sendEmail3("dmpickleball@gmail.com","Lesson Cancelled: "+user.name+" - "+fmtDateShort(lesson.date),user.name+" has cancelled their lesson on "+fmtDateShort(lesson.date)+" at "+lesson.time+".");
